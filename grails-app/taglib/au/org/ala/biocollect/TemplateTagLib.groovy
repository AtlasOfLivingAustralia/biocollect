...
class TemplateTagLib {
    ...
    private Map printLoginOrLogoutButton(Map hubConfig){
        if(!fc.userIsLoggedIn()){
            String loginUrl = grailsApplication.config.security.cas.loginUrl + "?service=" + grailsApplication.config.serverName + request.forwardURI + "?hub=" + hubConfig.urlPath
            return [displayName: 'Login', href: loginUrl, contentType:'external']
        } else {
            String logoutUrl = grailsApplication.config.grails.serverURL + "/logout/logout?casUrl=" + grailsApplication.config.security.cas.loginUrl + "/logout&appUrl=" + grailsApplication.config.serverName + request.forwardURI + "?hub=" + hubConfig.urlPath
