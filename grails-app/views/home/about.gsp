                ...
                <h1>${settingType.title?:'About the website'}
                    <g:if test="${fc.userInRole(role: grailsApplication.config.getProperty('security.cas.alaAdminRole')) || fc.userInRole(role: grailsApplication.config.getProperty('security.cas.adminRole'))}">
                        <span class="d-inline-block my-2">
                            <a href="${raw(g.createLink(controller:"admin",action:"editSettingText", id: settingType.name, params: [layout: 'bs5', returnUrl: raw(g.createLink(controller: params.controller, action: params.action, id: params.id, absolute: true))]))}"
        ...
        <div class="row">
            <div class="col-md-7">
                <div class="mt-3" id="aboutDescription">
                    <fc:markdownToHtml>${raw(content)}</fc:markdownToHtml>
                </div>
            </div>
            <g:if test="${showNews}">
            ...
            <div class="col-md-5">
                <g:if test="${fc.userInRole(role: grailsApplication.config.getProperty('security.cas.alaAdminRole')) || fc.userInRole(role: grailsApplication.config.getProperty('security.cas.adminRole'))}">
                    <a href="${raw(g.createLink(controller:"admin",action:"editSettingText", id: SettingPageType.NEWS.name, params: [layout: 'bs5', returnUrl: raw(g.createLink(controller: params.controller, action: params.action, absolute: true))]))}"
                       class="btn btn-sm btn-dark float-end"><i class="fas fa-pencil-alt"></i> Edit</a>
                </g:if>
                ${raw(newsText)}
            </div>
            </g:if>
