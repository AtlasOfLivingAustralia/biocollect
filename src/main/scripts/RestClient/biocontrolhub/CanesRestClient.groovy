...
def USERNAME = ""
def AUTH_KEY = ""
def xlsx = "canes_data_cleaned.xlsx"

//def PROJECT_ACTIVITY_ID = ""

def IMAGES_PATH = "images//canes//"
def DATA_TEMPLATE_FILE = "canes_data_template.json"
...
Paths.get(xlsx).withInputStream { input ->
    ...
    activities?.eachWithIndex { activityRow, activityIndex ->
        ...
        //if ((activityIndex >= 0 && activityIndex <= 0)) { // 183
            record = activityRow
            ...
            activity.outputs[0].data.observationId = record."observationId"//
            activity.outputs[0].data.recordType = record."recordType"//


            def constraint1= ["0","1 - 5","6 - 20","21 - 50","> 50"];
            def constraint2= ["None","Few canes","Several canes  - scattered over sample area","Several canes – localised on few plants","Numerous canes  - scattered on many plants over sample area","Numerous canes – localised on few plants","Many canes densely distributed over sample area", "Not specified"];

            if(!constraint2?.find{it == record."numberOfInfectedCanes"}) {
                println("Warning:: Constraint 2: (${activityIndex}) >> "+ record."numberOfInfectedCanes"  +" not in  constraint2 list")
            }
            ...
            if(!constraint1?.find{it == record."numberOfCanesExamined"}) {
                println("Warning:: Constraint 1: (${activityIndex}) >> "+ record."numberOfCanesExamined"  +" not in  constraint1 list")
            }

            switch(record."numberOfInfectedCanes") {
                case "":
                    activity.outputs[0].data.numberOfInfectedCanes =  "None" //
                    break;
                default:
                    activity.outputs[0].data.numberOfInfectedCanes = record."numberOfInfectedCanes"
            }

            activity.outputs[0].data.numberOfTargetPlantsExamined = record."numberOfTargetPlantsExamined"//
            activity.outputs[0].data.numberOfCanesExamined = record."numberOfCanesExamined"//
