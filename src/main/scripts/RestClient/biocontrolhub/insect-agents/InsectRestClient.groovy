...
def listTargetAgents = ["b6d004d3-752b-4348-be0b-1ee54c03611f", "8da35bbe-cccb-4e4c-bde6-2648f860cab3", "f23916da-1ca2-490b-a56c-13f5464e66ed", "e5914ac0-9d86-48d2-a725-86bd79b705d8"]
...
Paths.get(xlsx).withInputStream { input ->
    ...
    activities?.eachWithIndex { activityRow, activityIndex ->
        if ((activityIndex >= 2585 && activityIndex < activities?.size())) { // 183
            //if ((activityIndex >= 1 && activityIndex < activities?.size())) { // 183
            //sleep(2000)
            record = activityRow
            ...
            try{
                def SPECIES_URL = "/search/searchSpecies/" + record."BioCollect Survey ID"+ "?limit=1&output=Weed%20Biological%20Control%20-%20Agent%20Record%20for%20target%20species%20-%20Insect%20agents"
                // Get species name
            ...
            def constraint2 = ["0","1 - 5","6 - 20","21 - 50","51 - 200","> 200","Not specified"] // numberOfAgentsObserved
            ...
            if(!constraint2?.find{it == record."numberOfAgentsObserved"}) {
                println("Warning:: Constraint 2: (${activityIndex}) >> "+ record."numberOfAgentsObserved"  +" not in  constraint2 list")
            }
            ...
            activity.outputs[0].data.numberOfAgentsObserved = record."numberOfAgentsObserved"
