...
AEKOS.AekosViewModel = function (pActivityVM, projectViewModel, projectActivities, user, vocabList, projectArea) {
 ...
 /*   self.transients.ibraRegions = ko.observableArray([]);

    // Load IBRA 7 Regions
    self.loadIbraRegions = function () {
        $.ajax({
            url: fcConfig.spatialBaseUrl + "/ws/objects/cl1048",
            type: 'GET',
            async: false,
            timeout: 10000,
            success: function (data) {
                if(data.length > 0){
                    self.transients.ibraRegions (data);
                }
            },
            error: function (data) {
                console.log("Error occurred while calling layer service.", "alert-error", self.placeHolder);
            }
        });
    };

    self.loadIbraRegions();*/

    // default to the result of the intersection between the dataset location points with IBRA 7
    self.geographicalExtentDescription = ko.observable('');
    ...
    self.loadAekosData = function() {
        ...
        $.getJSON(url, function(data) {
            var features = aekosMap.generateDotsFromRecords(data)
            if (features && features.length > 0) {
                var lat = features[0].lat.toString();
                var lng = features[0].lng.toString();
                $.ajax({
